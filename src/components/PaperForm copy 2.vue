...
watch(() => gap.l, (gapL) => {
  console.log(`3: {gapL: ${gapL}}`);
  margin.left = gapL;
  margin.right = gap.x - gapL;
});
watch(() => gap.r, (gapR) => {
  console.log(`4: {gapR: ${gapR}}`);
  margin.right = gapR;
  margin.left = gap.x - gapR;
});
// watch(() => gap.y, (gapY) => {
...
//   margin.right = right;
// }, DebounceSeconds);
const resizeGapLeft = useDebounceFn((left) => {
  gap.l = left;
}, DebounceSeconds);
const resizeGapRight = useDebounceFn((right) => {
  gap.r = right;
}, DebounceSeconds);
</script>
<template>
